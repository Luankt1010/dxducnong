...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#060C1E] font-sans text-slate-100 antialiased selection:bg-blue-600 selection:text-white pb-16">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:py-10 space-y-10 sm:space-y-12">
        ...
        <section className="bg-gradient-to-r from-[#071630] to-[#0D2447] border border-blue-500/20 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-[0_0_20px_rgba(0,102,204,0.15)]">
          <div className="space-y-3 max-w-3xl text-center md:text-left">
            <h2 className="text-xl sm:text-2xl font-black text-blue-400 leading-tight uppercase tracking-wider">
          ...
          <button
            ...
            className="w-full md:w-auto px-6 py-3.5 bg-gradient-to-r from-[#00A3FF] to-[#0056B3] hover:brightness-110 text-white font-extrabold text-sm rounded-xl cursor-pointer shadow-[0_0_15px_rgba(0,163,255,0.4)] transition-all active:scale-95 text-center flex items-center justify-center gap-2 whitespace-nowrap"
          >
            <LayoutGrid size={16} />
        ...
        <div className="max-w-5xl mx-auto space-y-10 sm:space-y-12">
          ...
          <section className="space-y-6" ref={servicesHeadingRef}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-blue-950 pb-4">
              <div className="space-y-1.5">
                <h3 className="text-xl sm:text-2xl font-black text-white flex items-center gap-2.5">
                  <span className="p-1 px-2.5 bg-[#0056B3]/25 text-[#38BDF8] text-sm rounded-lg font-bold">4</span>
              ...
              <div className="flex flex-wrap gap-1.5 bg-[#0A1934] p-1 rounded-2xl border border-blue-900/40 self-start sm:self-center">
                {categories.map((cat) => (
                  <button
                    key={cat}
                    ...
                    className={`text-xs font-bold px-3 py-1.5 rounded-xl cursor-pointer transition-all ${
                      activeCategory === cat
                        ? 'bg-[#0056B3] text-white shadow-[0_0_10px_rgba(0,86,179,0.5)]'
                        : 'text-slate-400 hover:bg-[#0F264C] hover:text-white'
                    }`}
                  >
                    {cat}
                  </button>
      ...
      {/* 3. Footer */}
      <footer className="border-t border-blue-950 mt-16 bg-[#040A1A] py-8 text-center text-xs text-slate-500 space-y-3 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 space-y-2 relative z-10">
          <p className="font-bold text-slate-300 text-sm sm:text-base">
            © 2026 Đoàn Thanh niên xã Dục Nông, tỉnh Quảng Ngãi
          </p>
          <p className="max-w-2xl mx-auto text-[11px] sm:text-xs text-slate-400 leading-relaxed font-semibold">
          ...
          <div className="flex items-center justify-center gap-3 pt-2 text-slate-500">
